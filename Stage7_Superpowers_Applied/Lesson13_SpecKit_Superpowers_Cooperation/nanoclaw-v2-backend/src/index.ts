...
import { initDailyNews } from './modules/daily-news/index.js';
...
async function main(): Promise<void> {
  ...
  // 1b. Register daily-news recurring task in every active session that doesn't
  //     already have one. Idempotent — safe to run on every restart.
  ...
  for (const session of activeSessions) {
    ...
    if (fs.existsSync(dbPath)) {
      ...
      try {
        ...
        log.warn('Failed to init daily-news task', { sessionId: session.id, err });
      } finally {
        inDb.close();
      }
    }
