...
/** FR-009: 3 attempts with 1 s / 3 s / 10 s backoff. Calls onFinalFailure when all fail. */
export async function deliverText(
  client: Pick<WeChatClient, 'sendText'>,
  to: string,
  text: string,
  platformId: string,
  onFinalFailure: (err: unknown) => void,
): Promise<string | undefined> {
  const backoffs = [1000, 3000, 10000] as const;
  let lastErr: unknown;
  ...
  log.error('WeChat deliver failed after 3 attempts', { platformId, err: lastErr });
  ...
  factory: () => {
    ...
    const adapter: ChannelAdapter = {
      ...
      async deliver(
        ...
        return deliverText(client, to, text, platformId, () => {
          try {
            const today = new Date().toISOString().slice(0, 10);
            markAllFailedForDate(getDb(), today);
          } catch (dbErr) {
            log.warn('WeChat: failed to persist daily_news failure', { err: dbErr });
          }
        });
      },
    };
